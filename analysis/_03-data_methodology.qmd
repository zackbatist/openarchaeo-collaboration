...
```{r data-oarch}
# TODO: update and replace with local file before submission
...
We present an exploratory analysis of open-archaeo [[open-archaeo.info](open-archaeo.info), @batist2023], a directory of `r nrow(oarch)` <!-- TODO: make this consistent with what appears in Section 4, based on the filter --> pieces of open source archaeological software and other digital resources.
...
# Exclude from GitHub API data collection because does not have own repo
...
oarch_forges |>
  mutate(
    total = glue("{total} ({format(p_total * 100, trim = TRUE, digits = 1)}%)")
  ) |>
  select(-p_total) |>
  rename(Host = repo_host, Total = total) |>
  tt()
```
...
This included metadata on the GitHub repository and the programming languages it uses, as well as detailed data on each repository's individual contributors, commits, issues and comments.
...
The open-archaeo dataset and the supplemental data from the GitHub API provide us with a rich resource to explore the makeup of the archaeological software development scene.
We apply exploratory data analysis methods to break down and visualize the data, so that we may ascertain overall trends [@tukey1977].
In Section 4, <!-- encode section, see https://quarto.org/docs/authoring/cross-references.html#sections --> our focus is on examining the general state of open source archaeological software and resource development.
In Section 5, <!-- encode section --> we refine our analysis to examine development processes, with specific focus on collaborative experiences.
Finally, in Section 6, <!-- encode section --> we apply network analysis methods to investigate the formation of broader collaborative communities.
...
The quantitative analyses and figures presented here were generated with `r version$version.string` (@Manual).
The full data and code is available in the compendium that accompanies this paper (@Compendium).

...
We supplement this quasi-systematic collection strategy with word-of-mouth contributions made by interested individuals, who reached out via email, social media or at conferences to identify relevant work that we initially overlooked, including work that they created themselves.
...
Open-archaeo is also limited by the experiences of its primary maintainers, which affects the dataset's overall scope and how comprehensively it covers various domains of archaeological research.^[We welcome anyone, especially domain specialists who are familiar with the kinds of tools commonly used in their specific fields, to help fill in these gaps. Instructions for contributing to open-archaeo can be found at <https://github.com/zackbatist/open-archaeo>.]
...
```{r tbl-forges}
#| tbl-cap: Software forges used by open archaeology projects, as per open-archaeo
oarch_forges |>
  mutate(
    repo_host = recode(
      repo_host,
...
This means that we excluded projects that do not use version control (`r percent(p_no_vc)` of the total) or which develop and host code on platforms other than GitHub (`r percent(p_no_github)` of the total) from the parts of the analysis that examine *how* archaelogists develop software.
...
We did not directly observe or interview archaeological software developers, though our conclusions do draw heavily from our experience as members of that community ourselves.^[JR is an extremely prolific archaeological open source software developer, who actively builds code, teaches programming methods, and conducts code reviews. Through his maintenance of open-archaeo over the past 5+ years, ZB has honed an extensive understanding of the people who create archaeological software and the institutional ecology that supports their work.] 
...
Our analyses combine to support our objectives of understanding what kinds of software and resources archaeologists are making, how they create these tools in response to specific needs and use-cases, and how this work is situated within the context of an emerging community of practice.

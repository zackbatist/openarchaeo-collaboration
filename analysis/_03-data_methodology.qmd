...
However, there are a number of areas that, for practical reasons, are not well-covered.
<!-- TODO: next three sentences are a bit repeatative, can we more concisely state what OA doesn't cover well? -->
Code written before archaeologists started using collaborative software development platforms such as GitHub is generally missing.
...
We augmented the open-archaeo's list of with projects with information on its activity on GitHub, where applicable.
...
This data was collected via GitHub's public API using the R package `gh` [@csardi2023].

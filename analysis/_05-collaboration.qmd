...
The longest-lived projects have been active for between `english(round(lifespan_top10_range / 365)[1])` and `english(round(lifespan_top10_range / 365)[2])` years.
The most active projects see up to `round(max_commit_rate)` commits per day, but the majority of repositories (`r percent(p_commit_rate_1)`) receive less one commit per day.

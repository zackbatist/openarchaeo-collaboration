...
```{r fig-lifespan}
#| fig-cap: Lifespan of open archaeology repositories. Each point indicates a commit; excludes repositories with only one commit.
oarch_gh |>
  unnest(gh_commits) |>
  drop_na(datetime) |>
...
```{r fig-lifespan-rate}
#| fig-cap: Lifespan and commit rate of open archaeology repositories
oarch_lifespan %>%
  ggplot(aes(x = lifespan, y = commit_rate, fill = category, 0.3, size = n_contribs)) +
  geom_point(shape = 21, colour = "black", alpha = 0.7) +
  scale_x_log10() +
  scale_y_log10() +
  scale_fill_bright() +
  scale_size(trans = "log10") +
  labs(x = "Lifespan (days)", y = "Commits per day", 
       fill = "Category", size = "Contributors") +
  theme_oarch()
```
...
#| fig-cap: Box plot showing use of GitHub collaboration features in open archaeology repositories
oarch_gh |>
...
```{r fig-contributions}
#| fig-cap: Distribution of contributions in multi-contributor open archaeology repositories
...
This meshes with our prior observations that projects exhibiting higher commit rates have less contributors.
Additionally, our analyses neglect to account for contributions that are not tracked via git or GitHub.
...
We hypothesise that these findings show a preference for passive/reactive rather than active/proactive engagement with others' work.
This attitude is not conducive to achieving the desired outcomes of opening source code, namely enabling greater engagement, reuse and critique.
The technical apparatus provides the _potential_ for archaeologists to perform these actions, but these are inherently social activities, and as such, are circumscribed by social norms, including expectations and taboos surrounding the permeability of a grant-funded project's boundaries, or whether or not it is proper to actively engage or interfere with the work directed under another the aegis of another project, during or after the project's completion.
Our findings therefore suggest that there is no strong will to radically re-arrange the collaborative commitments that govern research practice.
If we want to make open source effective in relation to the aforementioned goals, we also need to foster a culture that supports active, pragmatic and humble critique, and which instills a de-territorialized attitude concerning what it means to contribute to collective knowledge.
This means fighting against the pathological power-relations that scaffold all aspects of academic life, and not fooling ourselves into believing that technical solutions (i.e. using git) will on their own resolve wicked social problems.
<!-- JR: this is a lot to take away from people being wary to make pull requests! How can we be sure that it isn't e.g. a question of limited time or technical capability? -->

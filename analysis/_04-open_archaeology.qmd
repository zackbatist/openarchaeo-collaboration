...
Our analysis of tags highlights aspects of archaeological work that software developers are inclined to contribute to [@tbl-tags].
...
oarch |>
  ...
  mutate(
    tag = glue("[{tag}](https://open-archaeo.info/{slugify(tag)})"),
...
p_gh_commits_cum <- oarch |>
  ...
  annotate("text", 
           ...
           label = "GitHub\nlaunched",
           vjust = 0) +
  scale_x_datetime(limits = c(ymd("2006-12-31", tz = "UTC"), now("UTC")),
                   date_breaks = "2 years", 
                   date_labels = "%Y",
                   expand = c(0,0)) +
...
p_gh_repos_cum <- oarch |>
  ...
  mutate(cum_commits = cumsum(n_commits)) |>
  select(category, date, cum_commits) |>
  group_by(category) |>
  nest() |>
  mutate(data = map(data, \(x) { # Extend the range of each category to today
    bind_rows(x, list(date = today(), cum_commits = max(x$cum_commits)))
  })) |>
  unnest(c(data)) |>
  ggplot(aes(date, cum_commits,
             fill = fct_rev(category),

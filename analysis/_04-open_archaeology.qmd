...
It includes both software and documents, but not research compendiums (collections of digital resources, including data, code, and documentation, which accompany or enhance a scientific publication; see [https://research-compendium.science](https://research-compendium.science).).^[see [https://github.com/benmarwick/ctv-archaeology#publications-that-include-r-code](https://github.com/benmarwick/ctv-archaeology#publications-that-include-r-code) for a similar list of archaeology publications that include R code]
Each record in open-archaeo is is assigned to a category based on how the tool or resource is meant to be accessed or used, and is annotated with tags that describe what aspect of archaeological research each itemtool or resource was meant to address.
Tags are ascribed based on how developers identified their projects’ purpose and scope, and each record can have multiple tags.
...
n_packages <- nrow(filter(oarch, category == "Packages and libraries"))
p_packages <- n_packages / nrow(oarch)
...
n_datasets <- nrow(filter(oarch, category == "Lists and datasets"))
...
From our breakdown of open-archaeo by category (see @tbl-categories), we can infer the pervasiveness of various development models, and the requisite technical capabilities that developers assume users hold.
Most resources (`r percent(p_packages)`) included in open-archaeo are designed to be used atop an existing "platform" -- for example a package that extends a programming language (e.g. radiocarbon calibration is implemented in R in the package 'rcarbon' or Python in the package 'iosacal') or a plugin for an application (e.g. 'ArchaeoAstroInsight' adds tools for measuring astronomical alignments to QGIS).
...
```
```{r data-platforms}
  p_platform_r <- sum(oarch$platform == "R", na.rm = TRUE) / sum(!is.na(oarch$platform))
  p_platform_python <- sum(oarch$platform == "Python", na.rm = TRUE) / sum(!is.na(oarch$platform))
  p_platform_qgis <- sum(oarch$platform == "QGIS", na.rm = TRUE) / sum(!is.na(oarch$platform))
  ```

Some `r percent(p_platform_r)` of projects are extensions to the statistical programming language R, making it the most widely-used platform by a large margin (@tbl-platforms).
Python, another programming language, is also relatively popular (`r percent(p_platform_python)`), as are plugins for the open source geographic information system QGIS (`r percent(p_platform_qgis)`).^[While Blender and QGIS plugins are written using the Python language, our intent while categorizing platforms was to get a sense of the developer ecosystems in which archaeological software engineers participate, rather than to simply gauge the popularity of different languages [@batist2023 2].]
...
These tools may require greater access to system resources, or may require more complex user interfaces than what R or Python IDEs (integrated development environments) tend to provide.

```{r tbl-tags}
#| tbl-cap: Themes of open archaeology projects. See Batist and Roe [-@batist2023 table 1] for a description of each tag's scope.
oarch |>
...
p_license_gpl <- n_filter(oarch, str_detect(license, "GPL")) / sum(!is.na(oarch$license))
p_license_mit <- n_filter(oarch, str_detect(license, "MIT")) / sum(!is.na(oarch$license))
p_license_cc <- n_filter(oarch, str_detect(license, "CC")) / sum(!is.na(oarch$license))
```
...
Two common free software licenses account for the majority of repositories that do contain licenses: the GNU General Public License (GPL, `r percent(p_license_gpl)`) and the MIT License (`r percent(p_license_mit)`).
...
p_gh_commits_cum <- oarch |>
  ...
  annotate("text", 
           ...
           label = "GitHub\nlaunched",
           vjust = 0) +
  scale_x_datetime(limits = c(ymd("2004-12-31", tz = "UTC"), dt_last_commit),
                   date_breaks = "2 years", 
                   date_labels = "%Y",
                   expand = c(0,0)) +
...
Archaeologists have been using git from at least the late 2000s, shortly after GitHub was launched,^[We excluded commits that have obviously erroneous dates, e.g. `2001-01-01` in projects started in the mid–late 2010s.].
but it saw a marked increase in popularity c. 2014--2015.
From that point and until recently there was an exponential uptake of GitHub by archaeologists,
but while we were preparing this paper (c. 2022) the first signs of a diminishing growth have appeared,
with the number of cumulative commits continuing to rise but the number of repositories hitting a sharp plateau.
This could be explained by market exhaustion, a shift in emphasis to maintaining existing code and working on established projects, and/or growing doubts about the appropriateness and sustainability of GitHub following its acquisition by Microsoft [@kansa2022].^[Concerns we very much share.]

GitHub's entry into the digital archaeology mainstream in 2014--2015 also marks the point at which we see it being used for things other than packaged source code (e.g. documents and scripts).
This suggests that the 'early adopters' of GitHub, pre-2014, were more directly embedded in existing (open source) software development communities,
while those that came later also saw version control systems as a potential medium for dissemination and archiving.
It may also reflect a general move towards git- and GitHub-based workflows by archaeologists attracted to open, participatory, and/or generally 'nerdy' academic practices.

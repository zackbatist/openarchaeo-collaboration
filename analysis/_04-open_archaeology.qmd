...
It includes both software and documents, but not research compendiums.^[see https://github.com/benmarwick/ctv-archaeology#publications-that-include-r-code for a similar list of archaeology publications that include R code]
...
Licenses are legally-bindings statements that stipulate how a creative work can be accessed and used.
...
These differ primarily in the restricts they place on reuse: the MIT License aims to be maximally permissive, while the GPL is a 'copyleft' license that specifies that all derivative works must be distributed under similar terms (in other words, it prohibits the use of FOSS software within non-FOSS software, [@cite?]).
...
Creative Commons licenses are a distant third place (`r percent(p_license_cc)` of repositories), in contrast to their widespread use for other forms of scholarly output [@cite?].
Many repositories do not specify a license; their maintainers may have assumed that making the code available, without explicitly stating permitting use, is enough to allow unrestricted access to the repository's contents.
This is evocative of a similar ignorance among academics regarding the unsustainable use of GitHub as a long-term code and data hosting platform [@milliken2021; @escamilla2022; @escamilla2023].
<!-- TODO: Seems like a pair of slightly uncharitable assumptions? -JR -->
<!-- TODO: (ZB) Expand with additional info after seeing what is in the "other" category -->
